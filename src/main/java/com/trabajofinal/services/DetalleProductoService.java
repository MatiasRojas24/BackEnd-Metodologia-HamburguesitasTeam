...
public class DetalleProductoService extends BaseService<DetalleProducto, String>{
    ...
    @Transactional
    public List<Producto> listarProductosPorTalleOrTipoProductoOrSexoOrPrecio(String idTalle, TipoProducto tipoProducto, Sexo sexo,Float minPrecio, Float maxPrecio)throws Exception{
        try{
            return detalleProductoRepository.findProductosByTalleOrTipoProductoOrSexoOrPrecio(idTalle,tipoProducto,sexo,minPrecio,maxPrecio);
        } catch (Exception e) {
            throw new Exception(e.getMessage());
        }
    }
